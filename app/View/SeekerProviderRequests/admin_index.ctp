<div class="seekerProviderRequests index">
    ...
    <div class="table-wrapper">
		...
		<tr>
			...
			//echo h($seekerProviderRequest['SeekerProviderRequest']['service_seeker_id']); ?>&nbsp;</td>
			<td><?php if(!empty($seekerProviderRequest['SeekerProviderRequest']['service_provider_id'])){
			...
			//echo h($seekerProviderRequest['SeekerProviderRequest']['service_provider_id']); ?>&nbsp;</td>
           <td><?php   $provider_category=$this->SmartForm->ProviderCategory($seekerProviderRequest['SeekerProviderRequest']['service_provider_id']);
			 $provider_category=str_replace(",",", ",$provider_category[0][0]['title']);
			// debug($provider_category);die;
			 echo $provider_category;
			 
		  /* foreach($this->SmartForm->getUserCategory($seekerProviderRequest['SeekerProviderRequest']['service_provider_id']) as $category):
			...
			<td class="actions">
                ...
                <?php if(empty($seekerProviderRequest['SeekerProviderRequest']['service_provider_id'])||($seekerProviderRequest['SeekerProviderRequest']['status']=='Withheld')){
							echo $this->Html->link(__('Reply'), array('action' => 'reply', $seekerProviderRequest['SeekerProviderRequest']['id'])); 
				}?>
	            
	            <?php if(empty($seekerProviderRequest['SeekerProviderRequest']['locked_by'])||($seekerProviderRequest['SeekerProviderRequest']['locked_by']==AuthComponent::user('id'))){?>
	            
	           <?php if($seekerProviderRequest['SeekerProviderRequest']['status']=='New'){
	           ...
	           <?php if($seekerProviderRequest['SeekerProviderRequest']['status']=='Assigned'){
	            ...
	            <?php if(empty($seekerProviderRequest['SeekerProviderRequest']['locked_by'])){
