 ...
 */
	...
	public function service_history($status=null) {
		...
		//debug($this->Paginator->paginate(array('service_seeker_id'=>$this->Auth->User('id'))));die;
			...
			$this->set('service_history', $this->Paginator->paginate(array('service_seeker_id'=>$this->Auth->User('id'),'status'=>$status)));
 ...
 */
	...
	//debug($this->request->data);die;
		if ($this->request->is('post')) {
			...
			if(array_key_exists("opt",$this->request->data['SeekerProviderRequest'])){
				...
				//debug($this->request->data);die;
						...
						if ($this->SeekerProviderRequest->save($this->request->data)) {
							...
							$this->send_mailto_admin(MAIL_FROM,$seeker_name);
			...
			$this->set(compact('serviceSeekers', 'serviceProviders', 'ratePackages','provider_rates'));
}
	
	private function send_mailto_admin($trilord_mail = null,$seeker_name=null){
			...
			$Email->viewVars(array('trilord_email'=>$trilord_mail,'seeker_name'=>$seeker_name));
	...
	private function send_mailto_seeker($seeker_email = null,$seeker_name=null){
			...
			$Email = new CakeEmail();
			$Email->config('default');
			$Email->viewVars(array('trilord_email'=>$trilord_mail,'seeker_name'=>$seeker_name,'policy'=>$policy,'user_agreement'=>$user_agreement));
			$Email->from(array($from  => 'TrilordMarket'))
				  ->to($to)
				  ->subject('Thank you')
				  ->emailFormat('html')
		...
		if ($this->request->is('post')) {
			...
			$seeker_email=$this->User->field('email',array('id'=>$this->Auth->User('id')));
				...
				if ($this->SeekerProviderRequest->save($this->request->data)) {
					...
					$this->send_mailto_trilord(MAIL_FROM,$seeker_name);
					$this->send_mailto_seeker($seeker_email,$seeker_name);
	...
	private function send_mailto_trilord($trilord_mail = null,$seeker_name=null){
			...
			if($to=='accept'){
					...
					if($remaining_amount >=$this->request->data['SeekerProviderRequest']['requested_amount']){
						...
						if ($this->SeekerProviderRequest->save($this->request->data)) {
							...
							$text_to_provider='Namaskar,Grahak le tapaiko sewa channu bhayeko cha.Tapailai haami aawashyak sewa barey samparka garchau. Dhanyabaad,TrilordMarket.';
							// build the url
 ...
 */
	...
	public function admin_verify($id = null,$number=null,$seeker_id=null,$provider_id=null){
		...
		if($number=='1'){
		$this->request->data['SeekerProviderRequest']['assigned_date']=date('Y-m-d');
		//debug($this->request->data['SeekerProviderRequest']['assigned_date']);die;
		$this->request->data['SeekerProviderRequest']['status'] = "Assigned";
		...
		//debug($this->SeekerProviderRequest->field('requested_amount'));die;
		$this->request->data['SeekerProviderRequest']['requested_amount']="";
			...
			//$user_phone=$this->User->query("select group_concat(primary_phone) primary_phone from users where id='$seeker_id'or id='$provider_id'");
			$seeker_phone = $this->User->field('primary_phone',array('id'=>$seeker_id));
			$provider_phone = $this->User->field('primary_phone',array('id'=>$provider_id));
			...
			$to_seeker =$seeker_mob_num; 
			$to_provider =$provider_mob_num;
			$text_to_seeker = 'Namaskar, tapaiko '.$category.' sewako order TrilordMarket ma confirm bhayo.Haami tapailai samparka garney chau.Dhanyabaad, TrilordMarket.';
			$text_to_provider='Namaskar,Grahak le tapaiko sewa channu bhayeko cha.Tapailai haami aawashyak sewa barey samparka garchau. Dhanyabaad,TrilordMarket.';
			// build the url
			
			if(!empty($seeker_mob_num)){
			
				$api_url_seeker =  "http://api.sparrowsms.com/call_in.php?" . 
                http_build_query(array(
                    "client_id" => $client_id,
                    "username" => $username,
                    "password" => $password,
				...
				$response1 = file_get_contents($api_url_seeker);
			}else{
			...
			//debug($this->request->data['SeekerProviderRequest']['completed_date']);die;
			...
			//$Seeker_deposit_detail = $this->Useful->availableAmount($seeker_id);
			//debug($this->request->data['SeekerProviderRequest']['completion_amount']);
			//debug($Seeker_deposit_detail['TotalDeposit']);
			//$remaining_balance=$Seeker_deposit_detail['TotalDeposit']-$this->request->data['SeekerProviderRequest']['completion_amount'];
			//debug($remaining_balance);die;
			...
			//$this->request->data['SeekerProviderRequest']['	withdrawn_date']=date('Y-m-d');
		...
		//$this->request->data['SeekerProviderRequest']['id'] = $id;
		/*
		if ($this->SeekerProviderRequest->save($this->request->data)) {
					$this->Session->setFlash('Success.','default',array('class'=>'success'));
					return $this->redirect(array('action' => 'index'));
