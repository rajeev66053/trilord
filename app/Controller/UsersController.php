 ...
 */
	...
	public $components = array('Paginator','Useful','PhpThumb.PhpThumb','PhpThumb.PhpCustom','RequestHandler');
...
public function login() {
	...
	if ($this->request->is('post')) {
		/*if($this->Auth->user()){
			if($this->Auth->user('registration_code')==''){
				return $this->redirect($this->Auth->redirect());
			}
			else{
				$this->Session->setFlash('You are not register yet');
			}
...
public function forgot_password(){
	...
	private function password_reset_mail($email = null,$link= null){
			$this->autoRender=false;
			$to = $email;
			$from = MAIL_FROM;
			...
			$Email->from(array($from  => 'Trilord Marketplace'))
				  ->to($to)
 ...
 */
	...
	public function provider_record() {		
		...
		if (!empty($this->params->query)) {
			...
			$conditions = array('role'=>'Serviceprovider');
			if (!empty($keyword)) {
				$conditions[] = array("OR" => array(
				"User.username LIKE '%$keyword%'",
				"User.permanent_address LIKE '%$keyword%'"
			),'role'=>'Serviceprovider');
			}
			if (!empty($Category)) {
			
			$serviceCategories = $this->User->query("SELECT GROUP_CONCAT(provider_service_categories.user_id) UserId FROM provider_service_categories where provider_service_categories.service_categories_id={$Category}");
				If(!empty($serviceCategories[0][0]['UserId'])){
					$conditions[] = array('User.id IN ('.$serviceCategories[0][0]['UserId'].')');
				}else{
					$conditions[] = array('User.id'=>'0');
				}
			}
			if (!empty($level)) {
				$conditions[] = array('User.expertise_level'=>$level,'role'=>'Serviceprovider');
			}
			$this->User->recursive = 0;
 ...
 */
	...
	public function seeker_edit() {
			...
			$existing_record=$this->User->find('first',array('recursive'=>'-1','fields' => array('name','email','primary_phone','dob_english','permanent_address','temporary_address'),'conditions'=>array('id'=>$id)));
			//$existing_record=$this->User->query("select name,email,primary_phone,dob_english,permanent_address,temporary_address from users where id='$id'");
			//debug($existing_record);
 ...
 */
 public function register() {
		...
		$cond=array('email'=>$email);
			...
			$text = 'Namaskar, Tapai TrilordMarket ma register hunu bhayeko cha.Toll Free number 1660-01-13579 Email:info@trilordmarket.com Dhanyabaad,TrilordMarket';
				...
				$this->Session->setFlash('SMS could not be sent.','default',array('class'=>'error-message'));
					...
					$this->set(compact('categories','service_packages','name'));
	...
	public function provider_profile($id = null){
		...
		$this->set(compact('user','ratePackages','serviceCategories','title_for_layout'));		
...
**/	
	...
	public function seeker_profile(){
		...
		$testimonial=$this->Testimonial->find('all',array('conditions'=>array('Testimonial.user_id'=>$this->Auth->User('id'),'Testimonial.is_active'=>'1'),
		'order' => 'Testimonial.created_date DESC'));*/
		...
		//$settings = $this->PaypalSetting->findById(1);
		
		$this->set(compact('history_1','history_2','history_3','deposit_record','complain_record','review_record','getDepositDetail','settings','testimonial','testimonial_existance'));	
...
//			}
			...
			$to =$provider_phone;
			$text = 'Welcome to Trilord Market';
...
**/	
	private function send_mail($email = null,$username = null,$name=null,$verify_code= null){
			...
			$Email = new CakeEmail();
			$Email->config('default');
			$Email->viewVars(array('verify_code' => $verify_code,'username'=>$username,'name'=>$name));
	...
	private function send_mailto_seeker($email = null,$user_name=null,$user_info=null){
			...
			$Email = new CakeEmail();
			$Email->config('default');
			$Email->viewVars(array('user_name'=>$user_name,'user_info' => $user_info,'trilord_email'=>$trilord_email,'policy'=>$policy,'user_agreement'=>$user_agreement));
			$Email->from(array($from  => 'TrilordMarket'))
				  ->to($to)
